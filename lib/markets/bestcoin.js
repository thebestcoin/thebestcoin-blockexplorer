...
function get_summary(coin, exchange, http_url, http_user, http_password, cb) {
  var req_url;

  if (http_user !== '' && http_password !== '') {
    req_url = url_scheme + http_user + ':' + http_password + '@' + http_url;
  } else {
    req_url = url_scheme + http_url;
  }
...
module.exports = {
  get_data: function (coin, exchange, http_url, http_user, http_password, cb) {
    var error = null;

    get_summary(coin, exchange, http_url, http_user, http_password, function (err, stats) {
      if (err) {
        error = err;
      }
